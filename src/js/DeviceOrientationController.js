...
var DeviceOrientationController = function ( object, domElement ) {
	...
	// white ++
	var tempDragQuat = new THREE.Quaternion();
	var lastDragQuat = new THREE.Quaternion();
	// end ++
	...
	this.onDocumentTouchStart = function ( event ) {
		...
		switch ( event.touches.length ) {
			case 1: // ROTATE
				...
				// white ++
				tempDragQuat = new THREE.Quaternion();
				// end ++
	...
	this.onDocumentTouchEnd = function ( event ) {
		...
		if ( appState === CONTROLLER_STATE.MANUAL_ROTATE ) {
			...
			if(tempDragQuat){
				lastDragQuat.multiply(tempDragQuat);
				// alert(lastDragQuat);
			}
			// end ++
	...
	var createQuaternion = function () {
		...
		return function ( alpha, beta, gamma, screenOrientation ) {
			...
			// finalQuaternion.multiply( lastDragQuat );
	...
	this.updateDeviceMove = function () {
		...
		return function () {
			...
			if ( alpha !== 0 && beta !== 0 && gamma !== 0) {
				...
				// white ++
				deviceQuat.multiply( lastDragQuat );
				// end ++
	...
	this.resetController = function () {
		lastDragQuat = new THREE.Quaternion();
	}
